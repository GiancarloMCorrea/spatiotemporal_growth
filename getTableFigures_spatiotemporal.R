...
#FigInx = 'SpatioTemp_1_C' # 1 is cohort and 2 is year. 3 is cohort_year
#listScenarios = c('D0-E0-F0-R3-cod', 'D3-E0-F0-R3-cod')
#scenario.labs <- c('Scenario 8', 'Scenario 9')
#listScenarios = c('D1-E0-F0-R2-cod')
#scenario.labs <- c('Scenario 7')
FigInx = 'SpatioTemp_1_C' # 1 is cohort and 2 is year. 3 is cohort_year
listScenarios = c('D0-E0-F0-R3-cod', 'D3-E0-F0-R3-cod')
scenario.labs <- c('Scenario 10', 'Scenario 11')


typeModel = 'cohort'
#typeModel = 'year'
#typeModel = 'cohort_year'

Wdth = 170 # 170 for 2 scenarios, 120 for 1 scenario
Hght = 200 # 200 for temporal

#FigInx = 'Temp_2'
#listScenarios = c('D2-E0-F0-R2-cod', 'D4-E0-F0-R2-cod')
#scenario.labs <- c('Scenario 7', 'Scenario 9')
...
ts.labs <- c("SSB", 'Recruitment', 'k')
names(ts.labs) <- c("SSB", "R", 'k')
  ...
  # if(typeModel == 'D0-R1'){
     ...
     ColorsX = c('blue', 'red', 'black')
         ...
         if(i == 3){
          ...
          # warning for EM:
           if(any(is.na(base_scalar[, selCols2[4]]))) {
            posNA = which(is.na(base_scalar[, selCols2[4]]))
            base_scalar[posNA, selCols2[4]] = base_scalar[posNA, selCols2[3]]
           }
...
#nAreas = 1 # in the OM
parameterNames = c('VonBert_K_Fem_GP')
...
alldata = list()
  ...
  if(typeModel == 'cohort'){
    ...
    selCols2 = c("VonBert_K_Fem_GP_1_om", "VonBert_K_Fem_GP_2_om",  
                 "VonBert_K_Fem_GP_1_em", "VonBert_K_Fem_GP_2_em", 
                 "CohortGrowDev_ENV_add_em")
    ...
    colnames(tmpDat) = c('scenario', 'parValueOM_1', 'parValueOM_2', 'parValueEM_1', 'parValueEM_2', 'parValueEM_ENV')
    tmpDat$parValueEM_ENV[is.na(tmpDat$parValueEM_ENV)] = 0

    trueSeries = read.csv('auxFiles/K_cohort_env.csv')
    trueSeries = trueSeries[trueSeries$years %in% yearseq, ]

    pdoSeries = read.csv('auxFiles/timeseriesgrowth_1.csv')
    pdoSeries = pdoSeries[pdoSeries$years %in% yearseq, ]

    areaVec = rep(rep(c(1,2), each = length(yearseq)), times = nrow(tmpDat))

    #alldata = NULL
    for(i in 1:nrow(tmpDat)){

      valsOM1 = tmpDat$parValueOM_1[i]*(1 + trueSeries$index)
      valsOM2 = tmpDat$parValueOM_2[i]*(1 + trueSeries$index)
      valsEM1 = tmpDat$parValueEM_1[i]*(1 + tmpDat$parValueEM_ENV[i]*pdoSeries$index)
      valsEM2 = tmpDat$parValueEM_2[i]*(1 + tmpDat$parValueEM_ENV[i]*pdoSeries$index)

      valsOM = c(valsOM1, valsOM2)
      valsEM = c(valsEM1, valsEM2)

      tmpdf = data.frame(years = yearseq, value_om = valsOM, value_em = valsEM, iteration = i,
                         scenario = tmpDat$scenario[i], area = areaVec)
      alldata[[i]] = tmpdf

    }

    alldata = do.call(rbind, alldata)
    alldata$parName = "VonBert_K_Fem_GP"
...
# FALTA MODIFICAR !!!!!!!!!!!!!!!!!!!!
  if(typeModel == 'year'){
    ...
    selCols2 = c("VonBert_K_Fem_GP_1_om", "VonBert_K_Fem_GP_2_om",  
                 "VonBert_K_Fem_GP_1_em", "VonBert_K_Fem_GP_2_em", 
                 'VonBert_K_Fem_GP_1_ENV_add_em', "VonBert_K_Fem_GP_2_ENV_add_em")
    ...
    if(any(is.na(base_scalar[, selCols2[4]]))) {
      ...
      tmpDat = base_scalar[ , c('scenario', selCols2)]
      colnames(tmpDat) = c('scenario', 'parValueOM_1', 'parValueOM_2', 'parValueEM_1', 'parValueEM_2', 
                            'parValueEM_ENV_1', 'parValueEM_ENV_2')
      tmpDat$parValueEM_ENV_1[is.na(tmpDat$parValueEM_ENV_1)] = 0
      tmpDat$parValueEM_ENV_2[is.na(tmpDat$parValueEM_ENV_2)] = 0

      trueSeries = read.csv('auxFiles/K_year_env.csv')
      trueSeries = trueSeries[trueSeries$years %in% yearseq, ]
      ...
      valsOM1 = tmpDat$parValueOM_1[i] + trueSeries$index
      valsOM2 = tmpDat$parValueOM_2[i] + trueSeries$index
      valsEM1 = tmpDat$parValueEM_1[i] + tmpDat$parValueEM_ENV_1[i]*pdoSeries$index
      valsEM2 = tmpDat$parValueEM_2[i] + tmpDat$parValueEM_ENV_2[i]*pdoSeries$index
...
alldata = transform(alldata, RE = ((value_em - value_om)/value_om)*100)
# new data to plot:
 par50 = aggregate(alldata$RE, list(year = alldata$years, scenario = alldata$scenario, parName = alldata$parName, area = alldata$area), 
                       FUN = median)
 par2_5 = aggregate(alldata$RE, list(year = alldata$years, scenario = alldata$scenario, parName = alldata$parName, area = alldata$area), 
                       FUN = quantile, probs = 0.025, na.rm = TRUE)
 par97_5 = aggregate(alldata$RE, list(year = alldata$years, scenario = alldata$scenario, parName = alldata$parName, area = alldata$area), 
                       FUN = quantile, probs = 0.975, na.rm = TRUE)
 alldata2 = data.frame(year = par50$year, scenario = par50$scenario, q50 = par50$x,
                       q5 = par2_5$x, q95 = par97_5$x, area = par50$area, type = 'k')

tsdataplot = rbind(tsdataplot, alldata2)

ColorsX1 = c('blue', 'red', 'black')
...
p5 = ggplot(tsdataplot, aes(x = year)) +
  ...
  geom_hline(yintercept = 0, linetype = "dashed", color = "black") +
  scale_color_manual(values = ColorsX) + 
  scale_fill_manual(values = ColorsX1) + 
  xlab("Year") + 
  ylab("Relative error %") +
  theme_bw() + 
...
#png(paste0('Figures/C_', FigInx, '_2.png'), width = 170, height = 200, units = 'mm', res = 500)
