...
om = "scenario_temporal_OM-Y/rock-om"
em = "scenario_temporal_OM-Y/rock-em"
scenarioName = 'D1-E0-F1-R1-rock'
...
G1 = list(L_at_Amax_Fem_GP_1 = growthindex1Linf$index) # check time varying parameters later
R2 = list(retro_yr = 0)
...
ml = meansize[meansize$SubSeas == 2 & meansize$Yr > iniFyear & meansize$Yr <= nyears, (5+age_env_obs_linf)] 
...
foreach(ix = 1:nSim) %dopar% {
  ...
  ss3sim_base(iterations = ix, scenarios = scenarioName,
              ...
              iniY = iniFyear, endY = nyears, age_env_obs = age_env_obs_linf)
...
E0 = list(natM_type = NULL, natM_n_breakpoints = NULL, natM_lorenzen = NULL, 
          ...
          par_int = c(base_natM), par_phase = c(6), forecast_num = 0) # This is not altering anything
# Standard (apply to OM):
G1 = list(VonBert_K_Fem_GP_1 = growthindex1Linf$index) # check time varying parameters later
R2 = list(retro_yr = 0)
