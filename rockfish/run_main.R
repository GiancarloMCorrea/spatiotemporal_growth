...
source('runScenario_temporal_Y_Linf.R') # probably we need to run this script internally because there are convergence problems for some iterations (errors)
source('runScenario_spatial_Linf.R') # probably we need to run this script internally because there are convergence problems for some iterations (errors)
...
scenNam = list.files()[grep(pattern = '-rock', x = list.files())]
...
# -----------------------------------------------------------------------------
# # -----------------------------------------------------------------------------
# # THIS CODE SHOULD BE RUN FOR EACH GROWTH INDEX PATTERN
# # First run these to get the bias_adjust parameters
# nSim = 25
# biasAdjust = TRUE
...
# averParams1 = matrix(NA, ncol = 5, nrow = length(scenNam))
# for(i in seq_along(scenNam)){
...
# 	averParams1[i, ] = colMeans(saveParms, na.rm = TRUE)

# }

# rownames(averParams1) = scenNam
# #write.csv(averParams, paste0('bias_adj_params_', growthTempIndex, '.csv'))
# averParams1 = read.csv(paste0('bias_adj_params_', growthTempIndex, '.csv'), row.names = 1)
# averParams1 = as.matrix(averParams1)
# labelsBiasAdj = c('#_last_yr_nobias_adj_in_MPD; begin of ramp', '#_first_yr_fullbias_adj_in_MPD; begin of plateau',
# 				  '#_last_yr_fullbias_adj_in_MPD', '#_end_yr_for_ramp_in_MPD (can be in forecast to shape ramp, but SS sets bias_adj to 0.0 for fcast yrs)',
# 				  '#_max_bias_adj_in_MPD (-1 to override ramp and set biasadj=1.0 for all estimated recdevs)')
...
# # -----------------------------------------------------------------------------
# # Now run the GetTableFigures script.