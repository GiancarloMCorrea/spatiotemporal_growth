...
FigInx = 'Temp_3' # 1 is cohort and 2 is year. 3 is cohort_year
#listScenarios = c('D3-E0-F0-R2-cod', 'D4-E0-F0-R2-cod')
#scenario.labs <- c('Scenario 3', 'Scenario 4')
...
Wdth = 120 # 170 for 2 scenarios, 120 for 1 scenario
...
scenariosNames = unique(base_scalar$scenario)
  ...
  if(typeModel == 'cohort'){
    ...
    selCols2 = c("VonBert_K_Fem_GP_1_om",  "VonBert_K_Fem_GP_1_em", "CohortGrowDev_ENV_add_em")
    tmpDat = base_scalar[ , c('scenario', selCols2)]
    colnames(tmpDat) = c('scenario', 'parValueOM', 'parValueEM', 'parValueEM_ENV')
    ...
    for(i in 1:nrow(tmpDat)){
      ...
      valsOM = tmpDat$parValueOM[i]*(1 + trueSeries$index)
      valsEM = tmpDat$parValueEM[i]*(1 + tmpDat$parValueEM_ENV[i]*pdoSeries$index)
      tmpdf = data.frame(years = yearseq, value_om = valsOM, value_em = valsEM, iteration = i,
                         scenario = tmpDat$scenario[i])
  ...
  if(typeModel == 'year'){
      ...
      selCols2 = c("VonBert_K_Fem_GP_1_om", "VonBert_K_Fem_GP_1_em", "VonBert_K_Fem_GP_1_ENV_add_em")
      tmpDat = base_scalar[ , c('scenario', selCols2)]
      colnames(tmpDat) = c('scenario', 'parValueOM', 'parValueEM', 'parValueEM_ENV')
      tmpDat$parValueEM_ENV[is.na(tmpDat$parValueEM_ENV)] = 0
      ...
      for(i in 1:nrow(tmpDat)){
        ...
        valsOM = tmpDat$parValueOM[i] + trueSeries$index
        valsEM = tmpDat$parValueEM[i] + tmpDat$parValueEM_ENV[i]*pdoSeries$index
        tmpdf = data.frame(years = yearseq, value_om = valsOM, value_em = valsEM, iteration = i,
                           scenario = tmpDat$scenario[i])
        alldata = rbind(alldata, tmpdf)
...
# new data to plot:
 par50 = aggregate(alldata$RE, list(year = alldata$years, scenario = alldata$scenario, parName = alldata$parName), 
                       FUN = median)
 par2_5 = aggregate(alldata$RE, list(year = alldata$years, scenario = alldata$scenario, parName = alldata$parName), 
                       FUN = quantile, probs = 0.025, na.rm = TRUE)
 par97_5 = aggregate(alldata$RE, list(year = alldata$years, scenario = alldata$scenario, parName = alldata$parName), 
                       FUN = quantile, probs = 0.975, na.rm = TRUE)
 alldata2 = data.frame(year = par50$year, scenario = par50$scenario, q50 = par50$x,
                       q5 = par2_5$x, q95 = par97_5$x, type = 'k')
