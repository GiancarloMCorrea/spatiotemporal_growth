...
growthindex_1 = read.csv(paste0('auxFiles/timeseriesgrowth_', 1, '.csv')) # REAL PDO: only this will apply in EM
#growthindex_2 = read.csv(paste0('auxFiles/timeseriesgrowth_', 2, '.csv'))

# THIS IS FOR YEAR EFFECT MODELS: -----------------------
...
growthindex1$index = growthindex1$index*base_K #Decide here
write.csv(growthindex1, 'auxFiles/K_year_env.csv', row.names = FALSE)
...
# THIS IS FOR COHORT EFFECT MODELS: ----------------------
Cgrowthindex1 = growthindex_1
#growthindex2 = growthindex_2

Cgrowthindex1$index = normalize(x = Cgrowthindex1$index, method = 'range', range = c(-CVgrowth, CVgrowth)) # apply to OM
Cgrowthindex1$index[1:iniFyear] = 0
write.csv(Cgrowthindex1, 'auxFiles/K_cohort_env.csv', row.names = FALSE)
...
png('Figures/Fplot_envindex.png', width = 100, height = 110, units = 'mm', res = 500)
...
# png('Figures/EnvIndexplot.png', width = 90, height = 110, units = 'mm', res = 500)
  par(mar = c(3,4,0,0.5))
  plot(growthindex_1$years, growthindex_1$index, type = 'l', xlab = '', ylab = 'PDO', axes = FALSE)
  axis(1)
  axis(2, las = 2)
